...
public static class PasswordResetEmailTemplate
{
    public static string Render(string username, string code) => $"""
        <div style="font-family: 'Segoe UI', Roboto, sans-serif; background: #313338; color: #dbdee1; padding: 32px; border-radius: 8px;">
          <h1 style="color: #f2f3f5; font-size: 20px; margin: 0 0 16px;">Redefinir senha</h1>
          <p style="margin: 0 0 16px;">Olá, {username}! Use o código abaixo para escolher uma nova senha no DiscClone:</p>
          ...
          <p style="margin: 0; color: #949ba4; font-size: 13px;">Esse código expira em {VerificationCodeGenerator.ValidityMinutes} minutos. Se você não pediu isso, pode ignorar este e-mail — sua senha continua a mesma.</p>
        </div>
        """;
}
