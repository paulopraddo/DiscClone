...
vi.mock('../lib/api')
vi.mock('../services/chatHub', () => ({ setAuthToken: vi.fn() }))
...
describe('AuthContext', () => {
  ...
  it('restaura o usuário do localStorage ao montar', () => {
    localStorage.setItem('discclone:auth', JSON.stringify({ userId: '1', username: 'joao', token: 'tok' }))
  ...
  it('login com sucesso define o usuário e persiste no localStorage', async () => {
    ...
    expect(result.current.user).toEqual({ userId: '1', username: 'joao', token: 'tok' })
    expect(JSON.parse(localStorage.getItem('discclone:auth')!)).toEqual({
      userId: '1',
      username: 'joao',
      token: 'tok',
    })
  ...
  it('logout limpa o usuário e o localStorage', async () => {
    ...
    expect(result.current.user).toBeNull()
    expect(localStorage.getItem('discclone:auth')).toBeNull()
  })
})
